...
const findWords = (board, words) => {
  ...
  const coord2idx = (x, y) => x + y * w;
  const visited = (mutate => {
    const set = new Set();
    return {
      add: (...args) => set.add(mutate(...args)),
      delete: (...args) => set.delete(mutate(...args)),
      has: (...args) => set.has(mutate(...args)),
    };
  })(coord2idx);
  ...
  words.map((word, i) => {
    const first = map.get(word.slice(0, 1))?.length || 0;
    const last = map.get(word.slice(-1))?.length || 0;
    if (last < first) {
      const rev = reverse(words[i]);
      reversed.set(rev, words[i]);
      words[i] = rev;
    }
  });
  const trie = Trie(words);
  const res = new Set();
  const loop = (trieNode, prefix = '', start = true, lx = 0, ly = 0) => {
    if (!trieNode) return;
    if (trieNode[0]) {
      trie.remove(prefix);
      res.add(prefix);
    }
    [...(trieNode[1] ?? new Map())].map(([char, childNode]) => {
      if (!map.has(char)) return;
      const posArr = map.get(char);
      posArr.map(([x, y]) => {
        if (visited.has(x, y)) return;
        const dist = start ? 1 : calcDist(x, y, lx, ly);
        if (dist !== 1) return;
        const nextWord = prefix + char;
        visited.add(x, y);
        loop(childNode, nextWord, false, x, y);
        visited.delete(x, y);
      });
    });
  };
  loop(trie.getData());
  return [...res].map(word => reversed.get(word) ?? word);
};
